...
const defaultCopyCases: ReadonlyArray<readonly [ScreenStateProps['state'], string, string]> = [
  ['loading', 'Đang tải dữ liệu', 'Vui lòng chờ trong giây lát.'],
  ['empty', 'Chưa có dữ liệu', 'Chưa có nội dung phù hợp để hiển thị.'],
  ['no-results', 'Không tìm thấy kết quả', 'Không có dữ liệu phù hợp với bộ lọc hiện tại.'],
  ...
  ['permission-denied', 'Bạn không có quyền truy cập', 'Hãy quay về khu vực được cấp quyền.'],
  ['offline', 'Đang ngoại tuyến', 'Nội dung hiện có có thể chưa phải dữ liệu mới nhất.'],
  ['stale', 'Dữ liệu có thể đã cũ', 'Nội dung này là lần cập nhật gần nhất đã nhận được.'],
  [
    'reconnecting',
    'Đang kết nối lại',
    'Nội dung hiện có vẫn được giữ trong khi khôi phục kết nối.',
  ],
  ['session-expired', 'Phiên làm việc đã hết hạn', 'Vui lòng đăng nhập lại để tiếp tục.'],
  ['conflict', 'Dữ liệu đã thay đổi', 'Hãy tải lại dữ liệu mới nhất trước khi tiếp tục.'],
];
...
describe('ScreenState state contract', () => {
  ...
  it('keeps the existing message override contract', () => {
    ...
    expect(screen.queryByText('Vui lòng thử lại khi kết nối ổn định.')).not.toBeInTheDocument();
  });
  ...
  it.each(['permission-denied', 'session-expired'] as const)(
    'never renders private children for %s',
    (state) => {
      render(
  ...
  it('keeps the authorized snapshot mounted with a conflict label', () => {
    render(
      <ScreenState state="conflict">
        <div>Snapshot đơn LP-0002</div>
      </ScreenState>,
    );

    expect(screen.getByText('Dữ liệu đã thay đổi')).toBeInTheDocument();
    expect(screen.getByText('Snapshot đơn LP-0002')).toBeInTheDocument();
  });
  ...
  it('supports a named action without overloading retry semantics', () => {
    ...
    render(<ScreenState state="no-results" actionLabel="Xóa bộ lọc" onAction={onAction} />);
