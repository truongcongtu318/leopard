...
describe("canAccess", () => {
  ...
  it("ADMIN can access fleet routes", () => {
    expect(canAccess("ADMIN", ["FLEET_OWNER"])).toBe(true);
  });

  it("ADMIN can access driver routes", () => {
    expect(canAccess("ADMIN", ["DRIVER"])).toBe(true);
  });
...
describe("requireRole", () => {
  ...
  it("ADMIN passes requireRole for any role combination", () => {
    expect(() => requireRole("ADMIN", ["ADMIN"])).not.toThrow();
    expect(() => requireRole("ADMIN", ["FLEET_OWNER"])).not.toThrow();
    expect(() => requireRole("ADMIN", ["DRIVER"])).not.toThrow();
    expect(() =>
      requireRole("ADMIN", ["ADMIN", "FLEET_OWNER", "DRIVER"]),
    ).not.toThrow();
  });
});
