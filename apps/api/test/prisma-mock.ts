...
export class InMemoryPrismaService {
  ...
  notification = {
    ...
    findUnique: jest.fn(async ({ where }: { where: { id: string } }) => {
      return this.notifications.get(where.id) ?? null;
    }),
    updateMany: jest.fn(
      async ({
        where,
  ...
  deviceToken = {
    upsert: jest.fn(
      async ({
        ...
        const existing = Array.from(this.deviceTokens.values()).find((t) => t.token === where.token);
        if (existing) {
          const updated = { ...existing, ...update } as DeviceToken;
          this.deviceTokens.set(existing.id, updated);
          return updated;
        }
        ...
        this.deviceTokens.set(id, token);
        return token;
      },
    ),
    findMany: jest.fn(async ({ where }: { where?: { userId?: string } } = {}) => {
      let list = Array.from(this.deviceTokens.values());
