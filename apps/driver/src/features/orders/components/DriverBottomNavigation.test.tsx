...
describe('DriverBottomNavigation', () => {
  it('renders all 4 tabs with correct accessibility labels', async () => {
    ...
    expect(screen.getByLabelText('Tôi')).toBeTruthy();
  ...
  it('marks the active tab as selected based on activeTab prop', async () => {
    ...
    const ordersTab = screen.getByLabelText('Đơn');
  ...
  it('navigates to /history, /earnings, /profile when pressing respective tabs from home', async () => {
    const onNavigate = jest.fn();
    const screen = await render(
    ...
    await fireEvent.press(screen.getByLabelText('Tôi'));
